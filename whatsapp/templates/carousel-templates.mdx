...
Open **Create New Template** and select **Carousel** as the category. Carousel is a Marketing-category template, so it's billed as Marketing on every send — see [How WhatsApp Message Pricing Works](/pricing-and-billing/whatsapp-pricing/overview).

If the editor then says the template is not supported for this channel, that number is not on Cloud API.
...
Submit the template to Meta. Approval follows the same process as other types. After **Approved**, wait about 15 minutes. See [Manage templates](/whatsapp/templates/manage-templates).
...
<AccordionGroup>
  <Accordion title="Carousel isn't available when I create a template.">
    Carousel templates require a Cloud API channel. If the number you picked is On-Premise, the editor shows **Carousel template is not supported for this channel**. Raise a ticket from that warning, or talk to support about moving to Cloud API.
  </Accordion>
  ...
  <Accordion title="My carousel template was rejected. Why?">
    Check card uniformity first. Mismatched media types or button layouts across cards are the usual cause. Then apply the usual Marketing rules: no misleading claims, and media that matches the text. See [Templates overview](/whatsapp/templates/overview#faqs).
  </Accordion>
...
- [Product marketing](/whatsapp/templates/product-marketing) (catalogue items, not uploaded cards)
