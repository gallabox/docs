...
> * Available on all Gallabox plans. Billed as an Authentication template on every send.

Authentication templates are only for one-time passwords and verification codes. Pick **Authentication template** in **Category**. WhatsApp writes the body. You cannot add a header, media, a coupon, or your own copy.
...
**Copy Code** here is not the [Coupon code](/whatsapp/templates/create-template#coupon-code) button on Custom marketing. Coupon copies a promo code. This copies an OTP.
...
1. Go to **WhatsApp → Templates**, click **Create New Template**, and pick **Authentication template**.
2. Name it, then choose the **Channel** and **Language**.
3. Under **Code Delivery Setup**, pick **Zero-tap Autofill**, **One-tap Autofill**, or **Copy Code**.
4. Fill **App Setup** if you picked Zero-tap or One-tap. Optionally add the security line and a code expiry.
5. Set the button labels, then click **Submit for approval**.
...
Once the template is **Approved**, wait about 15 minutes, then send it from a broadcast, a sequence, the inbox, a bot, or a test send.
...
<Accordion title="Is this the same as Coupon code?">
No. **Coupon code** on Custom marketing copies a promo code such as `GET50OFF`. **Copy Code** on an Authentication template copies the one-time password you pass in at send time.
</Accordion>
