...
A bot can also ask on its own. Add the **Request Phone Number** node to a Rule-Based Bot Flow and the bot sends the same prompt without an agent — see [Ask nodes](/ai-agents-and-bots/bots/nodes/ask-nodes#request-phone-number).

### Bots

Rule-based Bot Flows and the AI Bot both already recognize BSUID contacts and run a full flow without a phone number on file. Rule-Based Bot Flows can also request the number themselves with the [Request Phone Number node](/ai-agents-and-bots/bots/nodes/ask-nodes#request-phone-number).
...
<AccordionGroup>
  ...
  <Accordion title="Can my bot ask for the customer's phone number automatically?">
    Yes, in Rule-Based Bot Flows. Add the [Request Phone Number node](/ai-agents-and-bots/bots/nodes/ask-nodes#request-phone-number) and the bot sends WhatsApp's share-phone prompt itself. If the contact's number is already on file, the node skips itself and the flow carries on.

    An agent can still trigger it manually from the Inbox with **Request Phone Number** in the contact panel.
  </Accordion>
