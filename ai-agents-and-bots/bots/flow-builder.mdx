...
* A flow only runs in a conversation **assigned to its bot**. See [How conversations reach a bot](/ai-agents-and-bots/how-conversations-reach-a-bot)
* Each flow has a type that decides what starts it: **Message based** (a keyword), **Order based**, **WhatsApp Form based**, **AI Flow**, or **CTWA Ad Based** (a Click-to-WhatsApp ad)
...
* **Message based** — the customer's message matches a keyword ("hi", "help", "order"), or any message with **For any keyword**
* **Order based** — the customer places an order
* **WhatsApp Form based** — the customer submits a WhatsApp Form
* **AI Flow**
* **CTWA Ad Based** — the customer arrives from a Click-to-WhatsApp ad

Keywords match the whole message. See [How a flow is chosen](/ai-agents-and-bots/how-conversations-reach-a-bot#how-a-flow-is-chosen).
</Step>
...
* What happens if they don't respond? See [Bot wait time and drop-off](/ai-agents-and-bots/bots/wait-time-and-drop-off)
...
There's only one live version of a flow. When a change goes live depends on where you make it:

* **Changes on the canvas** (nodes and connections) go live when you click **Publish**.
* **Flow settings in Edit Flow** (keywords, nudges, finish actions, and flow-level drop-off) go live as soon as you save them on a published flow. You don't need to publish again.

Once a change is live, conversations already in progress continue on the updated flow from their current step. New conversations also start with the updated flow.

A new wait time applies from the bot's next question. A question that's already waiting keeps its old deadline.

Because in-progress conversations pick up your changes, avoid removing or heavily restructuring nodes that live conversations may be sitting on. Test changes before publishing.
</Accordion>
