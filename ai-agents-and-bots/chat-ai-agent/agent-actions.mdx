...
> _"If the customer provides their order number, use @apiCall to GET [https://yourshop.com/api/orders/\\\{order\\\_number\\\}](https://yourshop.com/api/orders/\{order\_number\}) and report the status."_
...
<AccordionGroup>
  <Accordion title="The @assignTo action is not working — what do I check?">
    ...
    2. Is the action name spelled correctly? Use @assignTo (not @assign or @assign_to).
