...
Go to **Settings → Workspace → AI Wallet**. The page shows credits remaining, credits used this month, a daily usage chart, a breakdown by feature, and a projected run-out date at current usage.

### Setting up alerts

Owners and Admins get an email when your balance drops below the low-balance threshold, and another when it reaches zero. The default threshold is 200 credits.

To change it, open **Low Balance Alert Settings** from the **Low balance alert** on the AI Wallet page and set a new threshold.

Alerts are sent by email only. The AI Wallet page also shows a warning when your balance is low or used up.

## When Credits Run Out

- **Chat AI Agent** ends the session. On WhatsApp and Instagram it sends your drop-off message and runs your drop-off actions. On Web Chat it resolves the chat. See [Why did the agent stop mid-conversation?](/ai-agents-and-bots/chat-ai-agent/faqs#agent-not-responding)
...
1. Go to **Settings → Workspace → AI Wallet** and click **Buy Credits**. **Buy Credits** isn't shown on trial accounts.
