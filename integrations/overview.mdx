...
**Example:** Bot flow asks how much the customer wants to pay → the Razorpay connector creates a payment link and stores it in a variable → a Send node delivers the link to the customer
...
* Pause a bot until the customer pays with WhatsApp Pay, then continue on a success or failure branch
* Send a WhatsApp template automatically when Razorpay, Stripe, or Cashfree reports a successful or failed payment
...
```
Bot flow: Wait for Order: "Browse our catalog and place your order to continue."
  → Ask WhatsApp Payment: customer pays inside WhatsApp
    → on success: Update Order Payment Status: paid
                  ...
                  Google Sheets: Add New Row with the order details
    → on failure: Send Text: "Your payment didn't go through. Please try again."
```

The **Ask WhatsApp Payment** node is the only node that waits for a payment. It needs [WhatsApp Pay](/whatsapp/payments#whatsapp-pay) connected. A payment link from Razorpay, Stripe, or PhonePe doesn't pause the bot — to confirm those payments, use the gateway's workflows. See [Payment Gateway integrations](/integrations/payment-gateways/overview).
