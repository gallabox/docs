...
**Catalog order → Shopify checkout link:**

```
Wait for Order: "Browse our catalog and place your order to continue."
  → Shopify: Create Draft Order from the customer's WhatsApp order
    → Store the Invoice URL in {{invoice_url}}
  → Send Text: "Complete your payment here: {{invoice_url}}"
```

The bot doesn't wait for the customer to pay the invoice — the flow moves on as soon as the link is sent.
...
```
Ask WhatsApp Payment: customer pays inside WhatsApp
  → on success: WooCommerce: Update order metadata (payment_status="paid", confirmed_via="WhatsApp bot")
...
## Complete Order Flow: WhatsApp Pay + HubSpot + Google Sheets

```
1. Wait for Order: "Browse our catalog and place your order to continue."
2. Ask WhatsApp Payment: customer pays inside WhatsApp
   → on success:
     a. Update Order Payment Status: paid
     b. HubSpot: Create or Update Contact
     c. Google Sheets: Add New Row with the order details
     d. Send Text: "Payment confirmed! Your order will ship within 24 hours."
   → on failure:
     Send Text: "Your payment didn't go through. Please try again."
```

**Ask WhatsApp Payment** is the only node that waits for a payment, and it needs [WhatsApp Pay](/whatsapp/payments#whatsapp-pay) connected. If you send a Razorpay or Shopify payment link instead, the bot doesn't wait for the payment. Use [Workflows for Razorpay](/integrations/payment-gateways/razorpay/workflows-for-razorpay) to confirm Razorpay payments automatically.
...
With WhatsApp Pay, the **Ask WhatsApp Payment** node continues on its failure branch:

```
Ask WhatsApp Payment
  → on success: Update Order Payment Status: paid
                Send Text: "Payment confirmed!"
  → on failure: Send Text: "We haven't received your payment. Please try again or contact support."
```

With a payment link, the bot isn't told when a payment fails. Use your payment gateway's **Payment Failed** workflow to message the customer instead. See [Payment Gateway integrations](/integrations/payment-gateways/overview).
</Accordion>
</AccordionGroup>
